...
# draw coords from x, y lists with pyplot
def draw(x: list, y: list):
    # get image
    datafile = cbook.get_sample_data(
        r'C:\Dev\Python\courses\next.py\Unit 6 - modules\final challenge\ex6p4.jpg')
    # read image with plt
    img = plt.imread(datafile)
    # scatter coords
    plt.scatter(x, y, zorder=1)
    # connect coords with a line
    plt.plot(x, y)
    # show the image
...
def main():
    ...
    for index, value in enumerate(first):
        if index % 2 == 0:
            first_x_lst.append(value)
        else:
            first_y_lst.append(value)
    ...
    for index, value in enumerate(second):
        if index % 2 == 0:
            second_x_lst.append(value)
        else:
            second_y_lst.append(value)

    # combine x, y lists
    combine_x = []
    combine_y = []

    # add first_x_lst and second_x_lst
    combine_x.extend(first_x_lst)
    combine_x.extend(second_x_lst)

    # add first_y_lst and second_y_lst
    combine_y.extend(first_y_lst)
    combine_y.extend(second_y_lst)

    # draw coords
    draw(combine_x, combine_y)
